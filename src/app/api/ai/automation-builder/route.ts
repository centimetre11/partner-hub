...
export async function POST(req: NextRequest) {
  ...
  const { messages, stream } = (await req.json()) as { messages: AutomationBuilderMessage[]; stream?: boolean };
  ...
  try {
    if (stream) {
      return createSseResponse(async (emit) => {
        const turn = await runAutomationBuilderTurn({ messages, userId: uid, emit, locale });
        emit({ event: "done", data: turn });
      });
    }
    const turn = await runAutomationBuilderTurn({ messages, userId: uid, locale });
