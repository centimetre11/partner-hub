            ...
            return (
              <div key={t.id} className="flex items-start gap-3 px-5 py-3.5 group">
                ...
                <form action={deleteTodoAction.bind(null, t.id)}>
                  <button className="text-zinc-200 hover:text-red-500 text-sm opacity-0 group-hover:opacity-100 transition-opacity">✕</button>
                </form>
              </div>
            );
          })}
          {todos.length === 0 && <EmptyState text="没有符合条件的待办" />}
        </div>
      </div>
    </div>
  );
