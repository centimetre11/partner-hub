  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-4 space-y-3 text-sm">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-slate-800">企业微信群</h3>
        {boundChat && (
      ...
      {boundChat ? (
        <div className="rounded-lg bg-slate-50 p-3 space-y-1 text-xs font-mono break-all">
          <div>
        ...
        <label className="block space-y-1">
          <span className="text-xs text-slate-500">群 Chat ID（在团队设置或服务器日志中查看）</span>
          <input
            value={chatId}
      ...
      <label className="block space-y-1">
        <span className="text-xs text-slate-500">群备注名（可选）</span>
        <input
          value={label}
      ...
      {boundChat && (
        <div className="border-t border-slate-100 pt-3 space-y-2">
          <span className="text-xs text-slate-500">测试推送到群（Markdown）</span>
          <textarea
            value={pushText}
