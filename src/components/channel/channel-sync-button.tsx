...
import { triggerChannelSyncAction } from "@/lib/channel-actions";
import { useMessages } from "@/lib/i18n/context";
import { formatMsg } from "@/lib/i18n/messages";

export function ChannelSyncButton() {
  const l = useMessages().channel;
  ...
  function syncNow() {
    startTransition(async () => {
      setMessage(null);
      setError(null);
      const res = await triggerChannelSyncAction();
      if ("error" in res && res.error) {
        setError(
          res.error === "SYNC_IN_PROGRESS"
            ? l.syncInProgress
            : res.error === "Channel sync failed"
              ? l.syncFailed
              : res.error,
        );
        return;
