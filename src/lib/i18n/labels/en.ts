import type { LabelsBundle } from "./types";

export const labelsEn: LabelsBundle = {
  locale: "en",
  pipelineStages: [
    { stage: 1, name: "Initial Contact", desc: "Exchange proposals, screen fit and mutual interest" },
    { stage: 2, name: "Active Progress", desc: "Real opportunities are in motion" },
    { stage: 3, name: "System Partnership", desc: "Batch pipeline, joint activities, system-level collaboration" },
  ],
  categoryLabels: {
    PURE_DATA: "Pure Data Consulting",
    POWER_BI: "Power BI Partner",
    TABLEAU: "Tableau Partner",
    QLIK: "Qlik Partner",
    IT_INTEGRATOR: "IT Integrator",
    OTHER: "Other",
  },
  industryLabels: {
    BANKING: "Banking & Finance",
    GOVERNMENT: "Government & Public",
    OIL_GAS: "Oil & Gas / Energy",
    RETAIL: "Retail & FMCG",
    MANUFACTURING: "Manufacturing",
    HEALTHCARE: "Healthcare",
    TELECOM: "Telecom",
    REAL_ESTATE: "Real Estate",
    LOGISTICS: "Logistics & Supply Chain",
    HOSPITALITY: "Hospitality & Travel",
    EDUCATION: "Education",
    MEDIA: "Media & Advertising",
    CROSS: "Cross-industry",
    OTHER: "Other / TBD",
  },
  capabilityLabels: {
    CONSULTING: "Consulting",
    IMPLEMENTATION: "Implementation",
    RELATIONSHIP: "Relationship",
    SALES: "Sales / Channel",
    TRAINING: "Training",
    INTEGRATION: "Integration",
  },
  poolFlagLabels: {
    NEW: "New Candidate",
    ADVANCING: "Advancing",
    WATCHING: "Watching",
    DROPPED: "Dropped",
  },
  statusLabels: {
    PROSPECT: "Prospect",
    ACTIVE: "Active Partner",
    ARCHIVED: "Archived",
  },
  contactRoleLabels: {
    APPROVER: "Approver",
    DECISION_MAKER: "Decision Maker",
    SUPPORTER: "Supporter",
    EVALUATOR: "Evaluator",
    INFLUENCER: "Influencer",
  },
  attitudeLabels: {
    3: "Coach",
    2: "Support (exclusive)",
    1: "Support (non-exclusive)",
    0: "Not contacted / Neutral",
    [-1]: "Opposed",
  },
  todoPriorityLabels: { HIGH: "High", MEDIUM: "Medium", LOW: "Low" },
  eventTypeLabels: {
    NOTE: "Note",
    MEETING: "Meeting Notes",
    CHAT_IMPORT: "Chat Import",
    AI_SUMMARY: "AI Summary",
    NEWS: "External Update",
    SYSTEM: "System",
    CHANGE: "Profile Change",
    MILESTONE: "Business Milestone",
  },
  businessRecordCategoryLabels: {
    VISIT: "Visit / Meeting",
    TRAINING: "Training / Certification",
    NEGOTIATION: "Negotiation",
    DELIVERY: "Delivery / Contract",
    RELATIONSHIP: "Relationship",
    OTHER: "Other",
  },
  crmTraceNatureLabels: {
    现场: "On-site",
    非现场: "Remote",
  },
  crmTraceActionLabels: {
    接待: "Reception",
    培训: "Training",
    服务: "Service",
    调研: "Research",
    方案: "Solution",
    催款: "Payment follow-up",
    客情: "Relationship",
    其它: "Other",
    远程会议: "Remote meeting",
    "WhatsApp or Line": "WhatsApp or Line",
    Email: "Email",
  },
  monitorDimensionLabels: {
    NEWS: "Company News",
    PEOPLE: "People Changes",
    HIRING: "Hiring Signals",
    DEALS: "Wins / Projects",
    FUNDING: "Funding / Finance",
    COMPETITOR: "Competitor Relations",
    SOCIAL: "Social Media",
    REPUTATION: "Reputation / Reviews",
    EVENTS: "Events / Conferences",
    ALLIANCE: "Ecosystem / Certifications",
    RISK: "Risk Alerts",
  },
  monitorSentimentLabels: {
    POSITIVE: "Positive / Opportunity",
    NEUTRAL: "Neutral",
    NEGATIVE: "Negative",
    RISK: "High Risk",
  },
  monitorSourceTypeLabels: {
    LINKEDIN: "LinkedIn",
    FACEBOOK: "Facebook",
    X: "X / Twitter",
    WEBSITE: "Website",
    NEWS: "News Source",
    CUSTOM: "Custom",
  },
  aiVerifiedLabels: {
    VERIFIED: "AI Verified",
    PARTIAL: "Partial Info",
    UNKNOWN: "Unverified",
  },
  documentTypeLabels: {
    AGENT_BRIEF: "Agent Brief",
    JOINT_SOLUTION: "Joint Solution Report",
    MEETING: "Meeting Notes",
    STRATEGY: "Strategy Analysis",
    CUSTOM: "Custom",
  },
  materialCategoryLabels: {
    TIER_POLICY: "Partner Tier Policy",
    PRODUCT_COMPARE: "Product Comparison",
    PITCH_DECK: "Pitch Materials",
    OTHER: "Other",
  },
  knowledgeCategoryLabels: {
    COMPANY: "Company Overview",
    STRATEGY: "Strategy & Policy",
    PRODUCT: "Product Capabilities",
    GTM: "Regional GTM Playbook",
    COMPETITOR: "Competitive Intelligence",
  },
  faqCategoryLabels: {
    PRODUCT_ADVANTAGE: "Product Advantages",
    COMPETITOR: "Competitor Comparison",
    PRICING: "Pricing & Commercial",
    INTEGRATION: "Integration & APIs",
    DELIVERY: "Implementation & Delivery",
    SECURITY: "Security & Compliance",
    OTHER: "Other Common Questions",
  },
  solutionStatusLabels: { DRAFT: "Draft", ACTIVE: "Active", ARCHIVED: "Archived" },
  partnerFieldLabels: {
    name: "Company Name",
    category: "Competitor Category",
    industries: "Primary Industries",
    capabilities: "Capability Tags",
    tier: "Tier",
    partnerArchetype: "Partner Archetype",
    valuePattern: "Joint Value Pattern",
    valuePartnerOffer: "Partner Offers",
    valueFanruanOffer: "Fanruan Offers",
    valueCustomerOutcome: "Customer Value",
    dedicatedHeadcount: "Dedicated Headcount",
    city: "City",
    country: "Country",
    headcount: "Company Size",
    website: "Website",
    companyType: "Company Type",
    coreBusiness: "Core Business",
    capability: "Core Capability",
    knownClients: "Known Clients",
    partnerAnnualRevenue: "Partner Annual Revenue (est.)",
    partnerDealsPerYear: "Deals Per Year (est.)",
    estimatedAnnualValue: "Est. Annual Value to FanRuan",
    certLevel: "Certification Level",
    currentTools: "Current BI Tools",
    keyDifferentiator: "Key Differentiator",
    playbook: "Core Playbook",
    pitch: "Pitch",
    bestChannel: "Best Contact Channel",
    fitScore: "Fit Score",
    priority: "Priority",
    pipelineStage: "Pipeline Stage",
    notes: "Notes",
  },
  partnerArchetypeLabels: {
    DATA_NATIVE: "Data-native",
    BI_MIGRATOR: "Competitor migration",
    IT_INTEGRATOR: "General IT integration",
    IOT_INTEGRATOR: "IoT / Smart city",
    SALES_AGENT: "Channel sales only",
    SHELL_DATA: "Shell data company",
    OTHER: "To be validated",
  },
  valuePatternLabels: {
    IOT_DASH: "IoT + Visualization",
    APP_REPORT: "Business systems + Complex reporting",
    CLOUD_APP: "Cloud channel + On-prem apps",
    DATA_BI: "Data governance + BI loop",
    BI_COMPLEMENT: "Competitor complement / Dual stack",
    OEM_EMBED: "OEM / Embedded",
    GOV_BID: "Government joint bidding",
  },
  customerSegmentLabels: {
    GOV_COMPLIANCE: "Government / Compliance",
    FINANCE: "Finance / Banking",
    RETAIL: "Retail / Consumer",
    ENERGY: "Energy / Oil & Gas",
    EDUCATION: "Education",
    HEALTHCARE: "Healthcare",
    MANUFACTURING: "Manufacturing",
    LOGISTICS: "Logistics",
    REAL_ESTATE: "Real Estate",
    OTHER: "Other",
  },
  buyingTriggerLabels: {
    COMPLIANCE: "Compliance-driven",
    MIGRATION: "Competitor migration",
    COMPLEX_REPORT: "Complex reporting",
    DATA_SOVEREIGNTY: "Data sovereignty",
    COST: "Cost optimization",
    DIGITAL_TRANSFORM: "Digital transformation",
  },
  entryPathLabels: {
    PARTNER_PURE_DATA: "Pure data consulting partner",
    PARTNER_POWER_BI: "Power BI partner",
    PARTNER_TABLEAU: "Tableau partner",
    PARTNER_IT_INTEGRATOR: "IT integrator partner",
    PARTNER_OTHER: "Other partner",
    DIRECT: "Direct sales",
  },
  icpTierLabels: {
    PRIMARY: "Primary ICP",
    NURTURE: "Nurture",
    WATCH: "Watch",
  },
  winFactorLabels: {
    PRODUCT_FIT: "Product fit",
    PARTNER_RELATION: "Partner relationship",
    PRICE: "Price advantage",
    COMPLIANCE: "Compliance capability",
    POC_SUCCESS: "POC success",
    RELATIONSHIP: "Customer relationship",
    OTHER: "Other",
  },
  lossReasonLabels: {
    COMPETITOR: "Lost to competitor",
    BUDGET: "Budget constraints",
    TIMING: "Bad timing",
    NO_FIT: "Product mismatch",
    PARTNER_ISSUE: "Partner issue",
    INTERNAL: "Customer internal reasons",
    OTHER: "Other",
  },
  actionDomainLabels: {
    COMMITMENT: "Organizational commitment",
    CAPABILITY: "Capability building",
    PIPELINE: "Pipeline advancement",
    RELATIONSHIP: "Relationship management",
  },
  userRoleLabels: {
    SALES: "Sales",
    PRESALES: "Pre-sales",
    ADMIN: "Super Admin",
    OTHER: "Other",
  },
  workspacePanels: [
    { id: "guide", label: "Overview", desc: "Status · Records · Todos" },
    { id: "positioning", label: "Positioning playbook", desc: "Tier · Type · Value pattern · Profile" },
    { id: "capability", label: "Capability building", desc: "Training · Drive materials · Joint solutions" },
    { id: "pipeline", label: "Pipeline", desc: "Pipeline opportunity tracking" },
    { id: "relationship", label: "Relationship management", desc: "Power map · Activity" },
  ],
  completenessLabels: [
    "City / country",
    "Company size",
    "Website",
    "Core business / capability",
    "Known clients",
    "Annual value estimate",
    "Current tools",
    "Joint value pattern",
    "Value proposition (3 lines)",
    "Partner archetype",
    "Target industries",
    "Dedicated headcount",
    "Core playbook",
    "Sales owner",
    "Pre-sales owner",
    "At least 1 key contact",
    "Decision maker identified",
    "Contact has contact info",
    "At least 1 tracked opportunity",
    "Activity in last 30 days",
    "Training / certification plan",
  ],
  taxonomyMeta: {
    ARCHETYPE: { label: "Partner archetype", hint: "How to engage" },
    INDUSTRY: { label: "Target industries", hint: "Which verticals (multi-select)" },
    VALUE_PATTERN: { label: "Joint value pattern", hint: "What to sell together" },
    CATEGORY: { label: "Competitive DNA", hint: "Background" },
    CAPABILITY: { label: "Capability tags", hint: "What they can do (multi-select)" },
    CUSTOMER_SEGMENT: { label: "Customer segment", hint: "End-customer ICP" },
    BUYING_TRIGGER: { label: "Buying trigger", hint: "Why they buy" },
    ENTRY_PATH: { label: "Entry path", hint: "How we reach them" },
    ICP_TIER: { label: "ICP tier", hint: "Segment priority" },
    WIN_FACTOR: { label: "Win factor", hint: "Why we won" },
    LOSS_REASON: { label: "Loss reason", hint: "Why we lost" },
  },
  fallbacks: {
    stage: "Stage {n}",
    attitude: "Not contacted / Neutral",
    tbd: "To be determined",
    unclassified: "Unclassified",
    notSelected: "Not selected",
    toBeWritten: "To be written",
    partiallyFilled: "Partially filled",
    captured: "Captured",
    ready: "Ready",
    noSales: "No sales",
    noPresales: "No pre-sales",
    dedicatedTbd: "Dedicated staff TBD",
    people: "people",
    active: "active",
    contacts: "contacts",
    activities: "activities",
    records: "records",
    jumpDown: "Jump ↓",
    clickView: "Click to view",
    clickEdit: " / edit",
  },
  frameworkLayerOrder: ["Positioning", "Playbook", "Actions", "Execution", "Stage exit"],
};
