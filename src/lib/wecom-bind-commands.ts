...
export function formatWecomBotHelpReply(): string {
  return [
    ...
    "• `@我 绑定` — 个人身份绑定说明",
    "• `@我 绑定 XXXXXX` — 个人中心生成的 6 位绑定码",
    "• `@我 绑定 CRM chenmin` — 设置 CRM 销售账号",
    "",
    "**群绑定（客户/伙伴）**",
    "• `@我 本群` — 查看本群绑定状态",
    "• `@我 绑定客户` → `@我 绑定客户 编号` — 从最近 10 个客户中选择",
    "• `@我 绑定伙伴` → `@我 绑定伙伴 编号` — 从最近 10 个伙伴中选择",
    "• `@我 绑定客户 XXXXXX` / `@我 绑定伙伴 XXXXXX` — Web 生成的群绑定码",
    "",
    "**Agent 创建（自动化工作流）**",
