...
const CHECKS: Check[] = [
  ...
  { labelKey: 12, weight: 3, ok: (p) => !!(p.salesUserId || p.ownerId) },
  { labelKey: 13, weight: 2, ok: (p) => !!p.presalesUserId },
  { labelKey: 14, weight: 10, ok: (p) => p.contacts.length > 0 },
  { labelKey: 15, weight: 8, ok: (p) => p.contacts.some((c) => c.role === "DECISION_MAKER") },
