...
const CHECKS: Check[] = [
  ...
  { labelKey: 5, weight: 5, ok: (p) => !!(p.estimatedAnnualValue || (p.partnerAnnualRevenue && p.partnerDealsPerYear)) },
  { labelKey: 6, weight: 5, ok: (p) => !!p.currentTools },
  { labelKey: 7, weight: 5, ok: (p) => !!p.valuePattern },
  { labelKey: 8, weight: 5, ok: (p) => !!(p.valuePartnerOffer && p.valueFanruanOffer && p.valueCustomerOutcome) },
  { labelKey: 9, weight: 4, ok: (p) => !!p.partnerArchetype && p.partnerArchetype !== "OTHER" },
  { labelKey: 10, weight: 4, ok: (p) => parseIndustries(p).some((c) => c && c !== "OTHER") },
  { labelKey: 11, weight: 4, ok: (p) => !!p.dedicatedHeadcount },
  { labelKey: 12, weight: 5, ok: (p) => !!p.playbook },
  { labelKey: 13, weight: 3, ok: (p) => !!(p.salesUserId || p.ownerId) },
  { labelKey: 14, weight: 2, ok: (p) => !!p.presalesUserId },
  { labelKey: 15, weight: 10, ok: (p) => p.contacts.length > 0 },
  { labelKey: 16, weight: 8, ok: (p) => p.contacts.some((c) => c.role === "DECISION_MAKER") },
  { labelKey: 17, weight: 4, ok: (p) => p.contacts.some((c) => !!(c.email || c.phone || c.contactInfo)) },
  { labelKey: 18, weight: 10, ok: (p) => p.opportunities.length > 0 },
  { labelKey: 19, weight: 8, ok: (p) => p.events.some((e) => Date.now() - new Date(e.createdAt).getTime() < 30 * 24 * 3600 * 1000) },
];
