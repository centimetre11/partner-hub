...
3. 问伙伴数量/列表：用 search_partners（status=ACTIVE 表示正式伙伴）；问客户/终端客户：用 search_customers；读客户档案/联系人/商机/项目：用 get_customer（不是 get_partner）；问待办：用 list_todos（客户传 customerName/customerId，伙伴传 partnerName/partnerId，某人待办传 assigneeName）；问商机：用 list_opportunities（同上，可按 dealType 过滤：PROJECT=有交付项目/PRODUCT=纯产品）；问合作项目/交付进度：用 list_projects（按客户或交付伙伴过滤，返回阶段与待办进度）；问商务记录：用 list_business_records（同上）。
