...
export async function listKmsChildPages(opts: {
  baseUrl: string;
  token: string;
  pageId: string;
  limit?: number;
  ...
  const data = await kmsFetch<{ results?: ConfluenceContent[] }>(
    opts.baseUrl,
    opts.token,
    `/rest/api/content/${encodeURIComponent(opts.pageId)}/child/page?${params}`,
  );
  return (data.results ?? []).map((item) => toKmsPage(opts.baseUrl, item));
}

function toAmmoKmsItem(page: KmsPage, parentTitle?: string): AmmoKmsItem {
  return {
    id: page.id,
    title: page.title,
    webUrl: page.webUrl,
    spaceName: page.spaceName,
    excerpt: page.plainText.replace(/\s+/g, " ").trim().slice(0, 300),
    updatedAt: page.updatedAt,
    parentTitle,
  };
}

export type AmmoKmsResult =
  | { ok: true; pages: AmmoKmsItem[] }
  | { ok: false; reason: "not_configured" | "kms_not_configured" | "error"; message: string };

export async function fetchAmmoKmsPages(
  urls: string[],
  userId: string | null | undefined,
): Promise<AmmoKmsResult> {
  if (!urls.length) {
    return { ok: false, reason: "not_configured", message: "No KMS page URLs configured" };
  }
  ...
  if (!cred?.accessToken) {
    return {
      ...
      message: "KMS token is not configured",
    };
  }

  const pages: AmmoKmsItem[] = [];
  const seen = new Set<string>();

  try {
    for (const url of urls) {
      const parent = await fetchKmsPageFromUrl({
        baseUrl: cred.baseUrl,
        token: cred.accessToken,
        url,
      });
      let children: KmsPage[] = [];
      try {
        children = await listKmsChildPages({
          baseUrl: cred.baseUrl,
          token: cred.accessToken,
          pageId: parent.id,
        });
      ...
      const pool = children.length > 0 ? children : [parent];
      for (const page of pool) {
        if (seen.has(page.id)) continue;
        seen.add(page.id);
        pages.push(toAmmoKmsItem(page, children.length > 0 ? parent.title : undefined));
      }
    }
...
export async function testAmmoKmsUrls(
  urls: string[],
  userId: string | null | undefined,
): Promise<{ ok: true; pageCount: number; sampleTitle: string | null } | { ok: false; error: string }> {
  const result = await fetchAmmoKmsPages(urls, userId);
  if (!result.ok) return { ok: false, error: result.message };
  return {
    ok: true,
    pageCount: result.pages.length,
    sampleTitle: result.pages[0]?.title ?? null,
  };
}
